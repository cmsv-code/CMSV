"""CMSV PyTorch Model Definitions
- CMSVMamba: MultiScale CNN + Mamba Hybrid

This module defines the Mamba-based model architecture for SV detection.
"""
...
class CMSVMamba(nn.Module):
    ...
    def forward(
        ...
        # --- Raw-token Mamba branch for the Mamba-only ablation ---
        ...
        raw_summary = raw_tokens.mean(dim=1)
        mamba_pred = self.mamba_classifier(raw_summary)
        ...
        cnn_sequence = x
        cnn_summary = cnn_sequence.mean(dim=1) + platform_context
        cnn_pred = self.cnn_classifier(cnn_summary)
